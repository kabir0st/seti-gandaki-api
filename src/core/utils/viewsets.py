...
from core.utils.functions import (export_data, export_data_from_response,
                                  get_all_exportable_fields, get_or_set_cache,
                                  get_unique_queryset)
...
class DefaultViewSet(ModelViewSet):
    ...
    def export(self, request, *args, **kwargs):
        ...
        human_readable_headers = request.GET.get('human_readable_headers',
                                                 'true').lower() == 'true'
        auto_exclude_non_human_fields = request.GET.get(
            'auto_exclude_non_human_fields', 'true').lower() == 'true'
        ...
        if hasattr(self, 'export_data') and callable(
            ...
            try:
                ...
                excel_file = export_data_from_response(data, document_name,
            ...
            start = int(request.GET.get('start', 0))
                ...
                request.GET['end']) if 'end' in request.GET and request.GET[
                    'end'] != 'None' else None
            ...
            try:
                if not hasattr(self, 'queryset') or self.queryset is None:
                    ...
                    if hasattr(list_response, 'data'):
                        ...
                        view_name = self.__class__.__name__.replace(
                            'ViewSet', '').replace('View', '')
                        ...
                        excel_file = export_data_from_response(
                            data, document_name, human_readable_headers)
                    else:
                ...
                raise APIException(
                    'There is a problem applying filters or getting data.'
                ) from e
        ...
        response[
            'Content-Disposition'] = f'attachment; filename="{document_name}.xlsx"'
