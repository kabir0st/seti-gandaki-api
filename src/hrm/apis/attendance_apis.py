...
class AttendanceViewSet(DefaultViewSet):
    ...
    def logs(self, request):
        ...
        attendances = Attendance.objects.filter(filter_query).order_by('date', 'time')
        ...
        for attendance in attendances:
            ...
            if date_key not in daily_attendances:
                daily_attendances[date_key] = {'check_in': None, 'check_out': None}
        ...
        for date_key, times in daily_attendances.items():
            ...
            if check_out_datetime > check_in_datetime:
                ...
                total_working_hours += working_duration.total_seconds() / 3600  # Convert to hours
        ...
        summary_data['Average-Hours-Per-Day'] = f"{total_working_hours / days_present if days_present > 0 else 0:.2f}"
        summary_data['Date-Range'] = f"{date_from or 'N/A'} to {date_to or 'N/A'}"
        ...
        if hasattr(response, 'data') and isinstance(response.data, dict):
            response.data['summary'] = {
                ...
                'average_hours_per_day': round(total_working_hours / days_present if days_present > 0 else 0, 2),
                'date_range': {
                    'from': date_from,
                    'to': date_to
                },
